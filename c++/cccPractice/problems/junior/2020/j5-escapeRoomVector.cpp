...
int main()
{
    ...
    while (!path.empty())
    {
        ...
        path.pop();
        if (find(visitedPositions.begin(), visitedPositions.end(), currentValue) != visitedPositions.end())
        {
            continue;
        }
        else
        {
            visitedPositions.push_back(currentValue);
            for (int i = 1; i * i <= currentValue; i++)
            {
                if (currentValue % i == 0)
                {
                    ...
                    if ((i <= numRows && newVal <= numColumns))
                    {
                        ...
                        if (find(visitedPositions.begin(), visitedPositions.end(), nextValue) == visitedPositions.end())
                        {
                            path.push(nextValue);
                        }
                    }
