...
int dijkstra(const vvi &grid)
{
    ...
    vvi cost(rows, vi(cols, INF));
    set<pair<int, pair<int, int>>> cells;
    cells.insert({grid[0][0], {0, 0}});
    cost[0][0] = grid[0][0];
    // pos cur;
    int r1, c1, cost1, r2, c2;
    while (!cells.empty())
    {
        auto cur = cells.begin();
        cells.erase(cur);
        ...
        REP(i, 0, 4)
        {
            ...
            if (r2 >= 0 && c2 >= 0 && r2 < rows && c2 < cols)
            {
                ...
                {
                    auto nxt = cells.find({cost[r2][c2], {r2, c2}});
                    if (nxt != cells.end())
                    {
                        cells.erase(nxt);
                    }
                    cost[r2][c2] = cost1 + grid[r2][c2];
                    cells.insert({cost[r2][c2], {r2, c2}});
                }
            }
        }
    }
