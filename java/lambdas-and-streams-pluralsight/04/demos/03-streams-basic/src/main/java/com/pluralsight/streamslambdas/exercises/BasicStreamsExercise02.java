...
public class BasicStreamsExercise02 {
	...
	/**
	 * Exercise 2a: In a list of products, find the products that are in a given
	 * category and return their names.
	 *
	...
	public List<String> getProductNamesForCategory(List<Product> products, Category category) {
		...
		return products.stream().filter(p -> p.getCategory().equals(category)).map(Product::getName).toList();
//        return products.stream()...;
	...
	/**
	 * Exercise 2b: Given a map that contains products grouped by category and a
	 * stream of categories, get the names of the products for each category in that
	 * stream and return all the products in a list.
	 *
	 * @param productsByCategory A map containing products grouped by category.
	 * @param categories         A stream of product categories.
	 * @return A list containing the names of products in each of the categories in
	 *         the given stream.
	 */
	public List<String> categoriesToProductNames(Map<Category, List<Product>> productsByCategory,
		...
		categories.forEach(c -> productsByCategory.get(c).stream().map(p -> p.getName())).collect(Collectors.toList());
//        return categories...;
