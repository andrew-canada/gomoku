...
public class FunctionalInterfacesExercise02 {
	...
	public Predicate<Product> createSearchPredicate(SearchCriteria criteria) {
		...
		/*
		 * TODO: Implement a lambda expression that checks if a product's category is
		 * equal to criteria.category
		 */
		Predicate<Product> categoryIs = p -> p.getCategory().equals(criteria.category);

		/*
		 * TODO: Implement a lambda expression that checks if a product's price is
		 * greater than criteria.minimumPrice
		 */
		Predicate<Product> minimumPriceIs = p -> p.getPrice().compareTo(criteria.minimumPrice) >= 0;

		/*
		 * TODO: Implement a lambda expression that takes a product and always returns
		 * true
		 */
