...
def check_and_update(grid, guess, word, row, letters):
    letter_in_word_count_dict = {k: [] for k in list(word)}
    letter_in_guess_count_dict = {k: [] for k in list(guess)}
    for i in range(5):
        letter_in_word_count_dict[word[i]].append(i)
        letter_in_guess_count_dict[guess[i]].append(i)

    for letter in letter_in_guess_count_dict:

        if list(word).count(letter) > 0:
            yellow_letter_array = letter_in_guess_count_dict[letter].copy()
            missing_letter_count = len(letter_in_word_count_dict[letter])
            for i in letter_in_guess_count_dict[letter]:
                if letter_in_word_count_dict[letter].count(i) > 0:
                    yellow_letter_array.remove(i)
                    missing_letter_count -= 1
                    grid[row][i] = white_on_green(guess[i])
                    # Color keyboard: Find the location of the current letter in the keyboard, then color it green
                    #if guess[:i+1].count(guess[i]) <= word.count(guess[i]):
                    letters[letters.index((list(filter(lambda x: x[10:11] == guess[i], letters)))[
                                            0])] = white_on_green(guess[i])
                else:
                    grid[row][i] = white_on_black(guess[i])
                    letters[letters.index((list(filter(lambda x: x[10:11] == guess[i], letters)))[
                                   0])] = white_on_black(guess[i])
            
            if len(yellow_letter_array) > 0 and missing_letter_count > 0:
                yellow_count = missing_letter_count if (len(yellow_letter_array) > missing_letter_count) else len(yellow_letter_array)
                for i in range(yellow_count):
                    grid[row][yellow_letter_array[i]] = white_on_yellow(guess[yellow_letter_array[i]])
                    letters[letters.index((list(filter(lambda x: x[10:11] == guess[yellow_letter_array[i]], letters)))[
                                 0])] = white_on_yellow(guess[yellow_letter_array[i]])
        else:
            for i in letter_in_guess_count_dict[letter]:
                grid[row][i] = white_on_black(guess[i])
                letters[letters.index((list(filter(lambda x: x[10:11] == guess[i], letters)))[
...
def get_statistics(num_games, num_wins, max_streak, current_streak, guess_distribution, newest_guess):
...
# Letter Board Functions
def render_letters(letters):
    print()
    for i in range(len(letters)):
        if (letters[i])[10:11] == "A":
            print(" ", end="")
        if (letters[i])[10:11] == "Z":
            print("    ", end="")

        print(letters[i], end=" ")

        if (letters[i])[10:11] == "P" or (letters[i])[10:11] == "L" or (letters[i])[10:11] == "M":
            print()
    print()


# Player Functions
def make_guess(GUESTED_WORDS):
    print("For help, enter \"help\"\n")
    print("Guested words: ", GUESTED_WORDS or {})
    guess = input("Guess a word: ").lower()
    
    if guess == "help":
        return "help"
    # The word occurs in the list
    GUESTED_WORDS.add(guess)
...
def display_board(grid, letters):
    os.system("cls")
    render(grid)
...
def main():
    ...
    while is_continue:
        ...
        for i in range(len(letters)):
            letters[i] = black_on_gray(letters[i])

        display_board(grid, letters)

        # Driver code for each game
        for i in range(6):
            guess = make_guess(GUESTED_WORDS)
            ...
            new_values = (check_and_update(grid, guess, word, i, letters))
            grid = new_values[0]
            letters = new_values[1]
            display_board(grid, letters)
        ...
        if is_see_statistics == "y":
            ...
            get_statistics(num_games, num_wins, max_streak,
                           current_streak, guess_distribution, newest_guess)
