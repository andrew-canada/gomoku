...
def check_and_update(grid, guess, word, row, letters):
    # Update the grid and the keyboard of letters
    for i in range(5):
        ...
        # More occurrences of the letter in the substring of the guess up to this point than in the actual word (color black)
            ...
            # More occurrences of the letter in the guess than in the actual word (color black)
            if guess.count(guess[i]) > word.count(guess[i]) and \
                    (guess.count(guess[i]) - (guess[:i+1]).count(guess[i]) != word.count(guess[i])-1):
                grid[row][i] = white_on_black(guess[i])
            else:
                grid[row][i] = white_on_yellow(guess[i])
            if guess[:i+1].count(guess[i]) <= word.count(guess[i]):
                if list(filter(lambda x: x[10:11] == guess[i], letters))[0] != white_on_green((list(filter(lambda x: x[10:11] == guess[i], letters))[0])[10:11]):
                    letters[letters.index((list(filter(lambda x: x[10:11] == guess[i], letters)))[
                        0])] = white_on_yellow(guess[i])
    # Returns the new version of the grid and the keyboard of letters
    return [grid, letters]
