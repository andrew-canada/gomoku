...
def right_wrong(guess: str) -> str:
    if word == guess:
        ...
        letter_check(user_guess)


def letter_check(guess: list) -> list:
    for i in range(5):
        if guess_list[i] in word_list:
            if guess_list[i] == word_list[i]:
                appender = (white_on_green(guess_list[i]))
                answer.append(appender)
                guess_list[i] = "1"
                word_list[i] = "1"
            else:
                answer.append(guess_list[i])
        elif guess_list[i] not in word_list:
            appender2 = (white_on_black(guess_list[i]))
            answer.append(appender2)
            guess_list[i] = "1"
    return answer


def yellower(guess: list) -> list:
    for i in range(5):
        if guess_list[i] in word_list and guess_list[i] != word_list[i] and guess_list[i] != "1":
            appender = (white_on_yellow(guess_list[i]))
            answer[i] = appender
            index = word_list.index(guess_list[i])
            word_list[index] = "1"
            guess_list[i] = "1"


def blacker(guess: list) -> list:
    for i in range(5):
        if guess_list[i] != "1":
            appender = (white_on_black(guess_list[i]))
            answer[i] = appender
...
def main():
    ...
    answers = [[" " for i in range(5)] for j in range(6)]
    ...
    while guess_counter <= 6:
        ...
        # print(word_list)
        if word == user_guess:
            print("You guessed it right! The word was {}".format(word))
            guess_counter = 6
            ...
            letter_check(user_guess)
            letter_check(guess_list)
            yellower(guess_list)
            blacker(answer)
            for elem in answer[0:5]:
                print(elem, end="")

        answers.insert(guess_counter, answer)
        make_grid(answers)
        clear()
        guess_counter += 1
        print(" Guesses left: {}".format(6-guess_counter))


while keep_playing == "Y" or "Yes" or "Ye" or "y" or "yes" or "ye":
    main()
